...
def parse_args():
    ...
    parser.add_argument('--pretrained_tokenizer_name_or_path', type=str, help="Path of the directory that contains the tokenizer")
    parser.add_argument('--pretrained_model_name_or_path', type=str, help="Path of the directory that contains the pretrained model")
    parser.add_argument('--batch_size', type=int, help="Batch size")
    parser.add_argument("--dropout", type=float, help="Value of dropout")
    parser.add_argument("--num_classes", type=int, help="Num of output classes for the classification task")
    parser.add_argument("--peak_lr", type=float, help="The Peak learning rate for the optimizer")
    ...
    parser.add_argument("--task_specific_layer_seed", type=int, default=random.randint(a=0, b=10000))
...
def main():
    ...
    train_dataloader = DataLoader(dataset=train_dataset, batch_size=config['batch_size'], shuffle=True, generator=generator)
    valid_dataloader = DataLoader(dataset=valid_dataset, batch_size=config['batch_size'], shuffle=False, generator=generator)
    ...
    for epoch in range(config["num_epochs"]):
        for batch in train_dataloader:
            batch_inputs = tokenizer(batch['text_excerpt'], padding=True, truncation=True, return_tensors='pt').to(device)
